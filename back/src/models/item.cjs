...
itemSchema.pre("validate", async function() {
    try {
        // Validation only if attributes are defined
        if (this.attributes && this.attributes.length !== 0) {
            ...
            if (ids.length !== new Set(ids).size) {
                return new Error(
                    "Duplicate attributes found in item's attributes array",
                );
            }
            ...
            for (const entry of this.attributes) {
                ...
                if (!attribute) {
                    return new Error(
                        `Attribute with ID ${entry.attribute} does not exist`,
                    );
                }
        ...
        if (this.subItems && this.subItems.length > 0) {
            this.subItems.forEach((subItemId) => {
                if (subItemId.toString() === this._id.toString()) {
                    return new Error("An item cannot be a sub-item of itself");
                }
            });
        }
    } catch (err) {
